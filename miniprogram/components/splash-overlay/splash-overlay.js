...
Component({
  ...
  lifetimes: {
    attached() {
      ...
      this._retryCount = 0
      this._played = false
      this._timer = setTimeout(() => this.finish(), MAX_DURATION)
      this.initVideo()
    },
  ...
  methods: {
    async initVideo() {
      ...
      try {
        ...
        console.error('[splash-overlay] prepare failed', error)
    ...
    scheduleAutoplayCheck() {
      ...
      this._autoplayTimer = setTimeout(() => {
        if (this._played || this.data.useFallback || this._finished) return
        this.setData({
          needTapPlay: true,
    ...
    async onVideoError(event) {
      console.error('[splash-overlay] video error', event.detail)
    ...
    enterFallback(statusText) {
      ...
      this._fallbackTimer = setTimeout(() => this.finish(), FALLBACK_DURATION)
    },

...
function buildBlendPrompt(promptText, negativePrompt = '') {
  const global = [
    ...
    '【角色风格】保持二次元平面图/表情包画风，不得变成3D模型、写实玩偶、真实猫狗或照片质感动物。',
    '【角色尺寸】手掌大小的小摆件尺度，高度约为画面高度的百分之五，最大不要超过百分之十，像小物件，不要巨大化。',
    '【允许调整】仅可微调姿势、表情、朝向与落点，并添加与场景一致的方向性高光和脚下接触阴影；不得改变角色辨识度。',
    '【构图】角色放在背景图空着的地方或前景空白处，不遮挡人脸。无水印无文字。',
    '【严格注意】图1是背景,逐像素保留,禁止任何修改。图2是角色素材，将其抠出后叠加到图1上,手掌大小。只允许微调表情和动作，禁止改变造型、配色、画风、体型。禁止重画背景、禁止重画角色、禁止生成真实动物、禁止画中画小窗。'
  ].join(' ')
